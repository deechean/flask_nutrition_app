...
def index():
    ...
    if request.method == "POST":
        date = request.form["date"]  # assume the date is in YYYY-MM-DD format
    ...
    cur = db.execute(
        """select log_date.entry_date, sum(food.protein) as protein, sum(food.carbohydrates) as carbohydrates, 
                         sum(food.fat) as fat, sum(food.calories) as calories from log_date left join food_date on 
                         food_date.log_date_id=log_date.id left join food on food.id=food_date.food_id 
...
@app.route("/view/<date>", methods=["POST", "GET"])  # date is going to be 20180520
def view(date):
    db = get_db()
    ...
    log_cur = db.execute(
        """select food.name, food.protein, food.carbohydrates, food.fat, food.calories 
                         ...
                         food.id=food_date.food_id where entry_date = ?""",
        [date],
    )
